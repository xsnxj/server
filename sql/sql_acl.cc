...
{
  if (lex_string_cmp(system_charset_info, &user->plugin,
                     &native_password_plugin_name) == 0)
    user->plugin= native_password_plugin_name;
  else
  if (lex_string_cmp(system_charset_info, &user->plugin,
                     &old_password_plugin_name) == 0)
...
applicable_roles_insert(ACL_USER_BASE *grantee, ACL_ROLE *role, void *ptr)
{
  ...
  if (!is_role)
  {
    if (data->user->default_rolename.length &&
        !lex_string_cmp(&data->user->default_rolename, &role->user))
                                           ...
                                           uchar **buff, ulong pkt_len)
{
  ...
  */
  if (lex_string_cmp(system_charset_info, &mpvio->acl_user->plugin,
  ...
  */
  if (mpvio.status == MPVIO_EXT::RESTART)
  {
    ...
    DBUG_ASSERT(command == COM_CHANGE_USER ||
                lex_string_cmp(system_charset_info, auth_plugin_name,
                               &mpvio.acl_user->plugin));
