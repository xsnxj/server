...
void QUICK_SELECT_I::add_key_name(String *str, bool *first)
{
  ...
  if (*first)
    *first= FALSE;
    ...
    str->append(',');
  str->append(key_info->name.str, key_info->name.length);
}
                                        ...
                                        bool *first)
{
  ...
  key_names->append(key_info->name.str, key_info->name.length);
                           ...
                           const char *msg)
{
  ...
  for (uint idx= 0; idx < param->keys; idx++)
  {
    if (tree_map->is_set(idx))
    {
      ...
      if (tmp.length())
        tmp.append(',');
      tmp.append(param->table->key_info[keynr].name.str);
    }
  }
  if (!tmp.length())
                                ...
                                struct st_ror_scan_info **end)
{
  ...
  for (;start != end; start++)
  {
    if (tmp.length())
      tmp.append(',');
    tmp.append(table->key_info[(*start)->keynr].name.str);
  }
  if (!tmp.length())
    tmp.append(STRING_WITH_LEN("(empty)"));
